...
__global__ void executeTests(const TestCase* __restrict__ testCases, 
    ...
    const half a = __ushort_as_half(tc.operandA);
    const half b = __ushort_as_half(tc.operandB);
    const half c = __ushort_as_half(tc.operandC);
    half res = __float2half(0.0f);
    
    // FP16硬件操作默认使用最近偶数舍入模式，忽略roundMode分支
    switch (tc.opcode) {
        case ADD:
            res = __hadd(a, c);
            break;
    ...
    // 存储结果
    results[idx].result = __half_as_ushort(res);
}
...
int main() {
    ...
    // 获取输入文件名
    while (true) {
        std::cout << "请输入输入文件名 (默认: fp16_input.txt): ";
        std::getline(std::cin, inputFilename);
        
        if (inputFilename.empty()) {
            inputFilename = "fp16_input.txt";
        }
    ...
    // 获取输出文件名
    std::cout << "请输入输出文件名 (默认: h100_fp16_output.txt): ";
    std::getline(std::cin, outputFilename);
    if (outputFilename.empty()) {
        outputFilename = "h100_fp16_output.txt";
    }
    ...
    std::cout << "H100 FP16 测试完成，结果已写入 " << outputFilename << std::endl;
    //std::cout << "编译建议: nvcc -arch=sm_90 -o FP16H100 FP16H100.cu\n";
    return 0;
}