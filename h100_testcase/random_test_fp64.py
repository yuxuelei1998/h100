...
def gen_operand_fp64():
    ...
    # 20% 为非数 (NaN)
        ...
        nan_types = [
            ...
            np.nan,                # numpy NaN
            # (尾数最高位=1)
            struct.unpack('d', struct.pack('Q', 0x7FF8000000000000))[0],
            # (尾数最高位=0且尾数≠0)
    ...
    # 60% 为常规数
        ...
        if random.random() < 0.8:
            ...
            while True:
                # 生成随机符号位
                sign_bit = random.getrandbits(1) << 63
                # 生成非零尾数(0x0000000000000001 - 0x000FFFFFFFFFFFFF)
                mantissa = random.getrandbits(52)
                if mantissa != 0:  # 确保非零
...
def float64_to_hex(f):
    ...
    # 转换常规数字
    uint_val = struct.unpack('Q', struct.pack('d', f))[0]
    return f"0x{uint_val:016X}"

def generate_fp64_test_cases(num_cases=2000):
    """生成FP64测试激励三元组"""
    test_cases = []
